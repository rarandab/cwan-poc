resource "aws_customer_gateway" "vpnclient" {
  ...
  bgp_asn    = var.vpn.asn_client
  ip_address = aws_eip.vpnclient.public_ip
  type       = "ipsec.1"

  tags = {
    Name = format("%s-%s-vpn-cgw", var.project_code, local.region_short_names[var.vpn.region])
  }
}
...
resource "aws_vpn_connection" "site2site" {
  ...
  tags = {
    Name = format("%s-%s-vpn-con", var.project_code, local.region_short_names[var.vpn.region])
  }
}
...
resource "aws_networkmanager_site_to_site_vpn_attachment" "site2site" {
  ...
  tags = {
    Name         = format("%s-%s-vpn-att", var.project_code, local.region_short_names[var.vpn.region])
